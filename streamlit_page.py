...
def test_linearity(
    ...
    feature_activations = (
        torch.rand(n_samples, n_chosen_feats) * (feat_max - feat_min)
    ) - feat_min  # n_samples x n_chosen_feats
    together_vectors = (
        feature_activations @ chosen_features
    )  # n_samples x n_chosen_feats @ n_chosen_feats x model_dimension = n_samples x model_dimension
    print(feature_activations.shape, together_vectors.shape, feature_activations.unsqueeze(-1).shape)
    single_feats = feature_activations.unsqueeze(-1) * torch.eye(
        n_chosen_feats
    )  # n_samples x n_chosen_feats x n_chosen_feats
    single_vectors = (
        single_feats.view(-1, n_chosen_feats) @ chosen_features
    )  # (n_samples x n_chosen_feats) x n_chosen_feats @ n_chosen_feats x model_dimension
    ...
    single_sums = single_output.view(n_samples, n_chosen_feats, model_dimension).sum(
        dim=1
    )  # n_samples x model_dimension
    ...
    projected_outputs = (
        together_output @ feature_vectors.T
    )  # n_samples x model_dimension @ model_dimension x n_feats = n_samples x n_feats
    ...
    interactions_score = interactions_reg.score(
        torch.cat([interactions, feature_activations], dim=1)[:test_n], projected_outputs[:test_n]
    )
    ...
    for i in range(n_attempts):
        ...
        quad_score = quad_reg.score(
            torch.stack([dir_projections_in, dir_proj_sq], dim=1)[:test_n], dir_projections_out[:test_n]
        )
...
def generate_dataset(
    ...
    if n_on is not None:
        ...
        assert (
            sparsity is not None and sparsity > 0 and sparsity < 1
        ), "Since n_on is not set, sparsity must be a float between 0.0 and 1.0"
...
def write_2d_and_gate():
    ...
    n1 = np.maximum(input_vals[:, 0] + input_vals[:, 1], 0.0)
    n2 = np.maximum(input_vals[:, 0] + input_vals[:, 1] - 1, 0.0)
...
def write_output_predictor():
    ...
    norm_power = st.slider("Norm power", 0.5, 20.0, 1.0, 0.1)
...
def write_old_intro():
    ...
    st.write(
        "The question I want to understand is how we should undertstand the relationship between feature formation and the nonlinearity of the neural network."
    )
    st.write(
        "In particular, I want to ask whether the formation of features needs to be in some way basis-aligned with the non-linearity."
    )
    ...
    st.write(
        "The way I'm going to look at neurons is through plotting 'response curves', where I vary the input along a particular direction, and plot the level of output along that direction."
    )
    st.write(
        "I will also plot the *input* in that direction. When there's no noise in the input, this is just the level of the feature, but as we start to add noise, it will be the level of the feature plus some noise."
    )
...
@st.cache_data
def make_nonhomog_grid(
    vector_dim: int,
    n_feats: int,
    bias_scale: float,
    nonlin_fn_str: str = "gelu",
    n_feats_tested: int = 10,
    n_samples: int = 5000,
):
    ...
    dims_per_feature_l = sorted(
        list(set([int(2**x) for x in np.linspace(1, np.log2(vector_dim), 20)] + [vector_dim]))
    )
...
def write_homogeneity_testing():
    ...
    logger.info(
        f"Running page with: Vector Dimension: {vector_dim} | Dims per Feature: "
    ...
    st.write(
        "The number of features required to get a homogeneity score above 0.1 and an interference score below 0.25"
    )
...
def write_linearity_test():
    ...
    input_pow = st.slider("Input Power", 0.5, 20.0, 1.0, 0.1)
    output_pow = st.slider("Output Power", 0.5, 20.0, 1.0, 0.1)
    ...
    inputs, outputs, vector_levels = generate_dataset(
        ...
        nonlin_fn=torch.nn.ReLU(),
        scale=5.0,
        biases=torch.rand(dimension) * 5.0,
        n_on=n_on,
    )
...
def write_lw_100_example():
    ...
    input_range = small_input_range * np.sqrt(
        50
    )  # use the sqrt of the number of elements to keep the scale of the non-linearity constant
